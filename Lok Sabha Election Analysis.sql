...
-- Total Seats Won by I.N.D.I.A. Allianz (India National  Development Inclusive  Alliance)
SELECT 
    SUM(CASE 
            WHEN party IN (
...
s.State
FROM 
constituencywise_results cr INNER JOIN partywise_results p ON cr.Party_ID = p.Party_ID
INNER JOIN statewise_results sr ON cr.Parliament_Constituency = sr.Parliament_Constituency
INNER JOIN states s ON sr.State_ID = s.State_ID
